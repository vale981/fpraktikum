...
Im Transmissionselektronenmikroskop werden Elektonen zur Bildgebung
genutzt, die sich aufgrund ihrer elektrischen Ladung durch Elektrische
Felder beschleunigen und durch Magnetfelder b\"undeln lassen.
...
Dabei kann es vorkommen, dass ein Ort in der Wölbung die Bragg-Bedingung (vgl.~\eqref{eq:bragg})
...
auch von der Fokusierung ab (siehe dazu~\ref{fig:simu}).
...
sich dem Fokus, so nimmt der Radius der Ringe zu. Ein Astigmatismus
  ...
  \begin{tabular}[htp]{l|ccc}
    ...
    &
      \begin{subfigure}{.29\textwidth}
        ...
        ]{../messungen/gold_hires/auswertung/6/zwei
          _richtungen/diag_6.jpg}%
        \caption{Gesamtbild.}
    ...
    &
      \begin{subfigure}{.29\textwidth}
        \centering
        \includegraphics[width=.8\textwidth]{../messungen/gold_hires/auswertung/6/zwei
          _richtungen/diag_6s.jpg}%
...
werden. Allerdings wurde nicht der Literaturwert, sondern die sich aus
dem ersten Peak \(\mqty(1&1&1)\) ergebende Gitterkonstante als
Referenz angenommen, damit eventuelle Kalibrierungsfehler des TEM nich
zu Abweichungen f\"uhren.  Die Netzebenenabst\"ande zu den einzelnen
Peaks sind in~\ref{tab:diffrnetz} aufgelistet und die zugeordneten
Werte f\"ur die Gitterkonstante k\"onnen in~\ref{tab:diffras}
eingesehen werden. Die dazugeh\"origen Netzebenen sind
in~\ref{tab:netzdiffr} aufgelistet.
...
\begin{equation}
  \label{eq:adiffr}
  a_{DIFFR} = \SI[parse-numbers=false]{0.3809\pm 0.001\,(sys)\pm 0.0052\,(stat)}{\nano\meter}
\end{equation}

Dieser Wert ist nicht innerhalb der Unsicherheitsgrenzen mit dem
Literaturwert vertr\"aglich. Die Messabweichungen wurden hier nicht
korrekt quantifiziert. Vermutlich liegt ein Kalibrierungsfehler am TEM
vor. Die Abweichungen sind numerisch kleiner als bei der direkten
Vermessung der Netzebenen. Der Erwartung nach sollte die Bestimmung
der Gitterkonstante aus dem Beugungsbild genauer sein, da hier der
Drift der Probe, ihre Ausrichtung und die Fokussierung keine so große
Rolle spielen.  Die direkte Vermessung der Netzebenenabst\"ande ist
zwar instruktiv, aber der Messung aus dem Beugungsbild hier in der
systematischen Abweichung unterlegen. Auch l\"asst sich die Vermessung
des Beugungsbildes besser automatisieren. Die Abh\"angigkeit von der
Kalibrierung ist hier aber ein Problem.
