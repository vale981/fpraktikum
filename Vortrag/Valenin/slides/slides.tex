...
\setbeamertemplate{note
  page}{\begin{columns}\column{.5\textwidth}\insertslideintonotes{0.5}\column{.5\textwidth}\insertnote\end{columns}}
...
\begin{frame}{Versuchsziel und Aufbau}
  \begin{columns}
    ...
    \end{figure}
    \column{.5\textwidth} \only<1,2>{
      \begin{block}{Ziel des Versuches}
        Jusierung, Inbetriebname und Untersuchung eines \hne{} Lasers.
      \end{block}
    } \only<2,3>{
      \begin{block}{Zur verf\"ugung stehendes Material}
        \begin{description}
          ...
          \item[Blenden] Als Justagehilfe und zum Ausblenden von
            unerw\"unschten Moden.  } \only<3>{
          \item[Linsen und Filter] Zur Untersuchung der
            Strahleigenschaften. (Sammellinse, Polfilter, Graufilter)
          \item[Fabry Perot Interferometer] Festaufbau, Konfokal
          \item[Leistungsmessger\"at] Zur Leistungsmessung und als
            Justagehilfe.
          \item[Faserspektrometer] \textsc{Ocean Optics HR2000+} als
            Referenzmessger\"at.  }
        \end{description}
      \end{block}
    }
...
\begin{frame}
  \begin{columns}
    ...
    \item<1-> Vielstrahlinterferenz durch Reflexion zwischen zwei
      ebenen Spiegeln (Etalon)\note<1->[item]{wieder ein Resonator}
      \begin{itemize}
...
\begin{frame}
  \note[item]{Vor dem Versuch} Da \(g_1(R_1=\infty)=1\) folgt mit
...
\begin{frame}
  ...
  \uncover<4->{
    \begin{table}
      \begin{tabular}{l|SSSS}
        ...
        \bottomrule
      \end{tabular}
      \caption{Leistungsmessungen des Einfachdurchgangs mit
  ...
  \end{itemize}
  \note<3->[item]{zu wenig zeit f\"ur noch laenger}
  \note<4->[item]{Syst. vernachl. fehler aus stat}
  \note<6->[item]{Notwendigkeit Resonator}
\end{frame}
...
\begin{frame}{Betrachtung der Linienverbreiterung}
  ...
  \note<2->[item]{da andere Mechanismen vernachl. eher zu hohe temp erwartet!}
\end{frame}
